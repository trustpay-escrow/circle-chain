import { Award, ShieldCheck, CheckCircle2, History } from 'lucide-react';

export default function ProfilePage() {
  const user = {
    address: 'GABC1234567890WXYZ1234567890',
    displayName: 'Zion Adebayo',
    reputationScore: 98,
    circlesCompleted: 4,
    punctualContributions: 24,
    totalCollateralStaked: 150,
  };

  return (
    <div className="space-y-8 max-w-4xl mx-auto py-6">
      {/* Profile Header */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <p className="text-xs font-mono text-slate-400 mt-1">{user.address}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2">
          ...
          <span className="text-3xl font-extrabold text-white block">{user.circlesCompleted}</span>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2">
          ...
          <span className="text-3xl font-extrabold text-white block">{user.punctualContributions}</span>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2">
          ...
          <span className="text-3xl font-extrabold text-white block">${user.totalCollateralStaked} USDC</span>
